...
from programy.storage.entities.category import (
    CategoryReadOnlyStore,
    CategoryReadWriteStore,
)
...
class CategoryReadOnlyStoreTests(unittest.TestCase):
    ...
    def test_load_category(self):
        ...
        match = parser.match_sentence(
            client_context, Sentence(client_context, "HELLO"), "*", "*"
        )
    ...
    def patch_parse_category2(
        self, category_xml, namespace, topic_element=None, add_to_graph=True, userid="*"
    ):
        raise Exception("Mock Exception")

    @patch(
        "programy.parser.aiml_parser.AIMLParser.parse_category", patch_parse_category2
    )
...
class CategoryReadWriteStoreTests(unittest.TestCase):
    ...
    def test_store_category(self):
        ...
        with self.assertRaises(NotImplementedError):
            category_store.store_category(
                "groupid", "userid", "topic", "that", "pattern", "template"
            )

    def test_upload_from_file_categories_only(self):
        store = MockCategoryReadWriteStore()
        filename = (
            os.path.dirname(__file__) + os.sep + "categories" + os.sep + "fife.aiml"
        )
        count, success = store.upload_from_file(
            filename, fileformat=Store.XML_FORMAT, commit=True, verbose=False
        )
    ...
    def test_upload_from_file_with_other(self):
        store = MockCategoryReadWriteStore()
        filename = (
            os.path.dirname(__file__) + os.sep + "categories" + os.sep + "other.aiml"
        )
        count, success = store.upload_from_file(
            filename, fileformat=Store.XML_FORMAT, commit=True, verbose=False
        )
    ...
    @patch(
        "programy.storage.entities.category.CategoryReadWriteStore.store_category",
        patch_store_category,
    )
    def test_upload_from_file_categories_only(self):
    ...
    @patch(
        "programy.storage.entities.category.CategoryReadWriteStore._upload",
        patch_upload,
    )
