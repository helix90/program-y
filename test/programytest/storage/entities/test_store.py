...
class StoreTests(unittest.TestCase):
    ...
    def test_split_into_fields(self):
        ...
        self.assertEquals(
            ["FIELD1", "FIELD2"], store.split_into_fields("FIELD1,FIELD2")
        )
        self.assertEquals(
    ...
    def test_get_file_processor_text(self):
        ...
        self.assertIsInstance(
            store.get_file_processor(
                Store.TEXT_FORMAT, os.path.dirname(__file__) + os.sep + "test.txt"
            ),
            TextFile,
        )
    ...
    def test_get_file_processor_csv(self):
        ...
        self.assertIsInstance(
            store.get_file_processor(
                Store.CSV_FORMAT, os.path.dirname(__file__) + os.sep + "test.csv"
            ),
            CSVFileReader,
        )
    ...
    def test_get_file_processor_other(self):
        ...
        with self.assertRaises(Exception):
            self.assertIsInstance(
                store.get_file_processor(
                    "other", os.path.dirname(__file__) + os.sep + "test.csv"
                ),
                CSVFileReader,
            )
    ...
    def test_upload_from_file_no_commit(self):
        store = MockStore(throw_except=False)
        final_count, final_success = store.upload_from_file(
            os.path.dirname(__file__) + os.sep + "test.txt", commit=False
        )
        self.assertEquals(1, final_count)
        self.assertEquals(0, final_success)
        self.assertTrue(store.processed)
    ...
    @patch(
        "programy.storage.entities.store.Store.get_file_processor",
        patch_get_file_processor,
    )
    ...
    def test_upload_from_directory_with_subdir(self):
        ...
        final_count, final_success = store.upload_from_directory(
            os.path.dirname(__file__) + os.sep + "testdir", subdir=True
        )
    ...
    def test_upload_from_directory_with_subdir_no_commit(self):
        store = Store()
        final_count, final_success = store.upload_from_directory(
            os.path.dirname(__file__) + os.sep + "testdir", subdir=True, commit=False
        )
        self.assertEquals(5, final_count)
        self.assertEquals(0, final_success)
    ...
    def test_upload_from_directory_without_subdir(self):
        store = Store()
        final_count, final_success = store.upload_from_directory(
            os.path.dirname(__file__) + os.sep + "testdir", subdir=False
        )
        self.assertEquals(3, final_count)
        self.assertEquals(0, final_success)
    ...
    def patch_upload_from_file(
        self, filename, fileformat=Store.TEXT_FORMAT, commit=True, verbose=False
    ):
        raise Exception("Mock Exception")

    @patch(
        "programy.storage.entities.store.Store.upload_from_file", patch_upload_from_file
    )
