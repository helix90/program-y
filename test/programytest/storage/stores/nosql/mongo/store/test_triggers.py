...
class MongoTriggerStoreTests(TriggersStoreAsserts):
    ...
    def test_initialise(self):
        ...
        engine.initialise()
        store = MongoTriggerStore(engine)
    ...
    def patch_load_triggers_from_file(self, filename, verbose):
        raise Exception("Mock Exception")

    @unittest.skipIf(Engines.mongo is False, Engines.mongo_disabled)
    @patch(
        "programy.storage.stores.nosql.mongo.store.triggers.MongoTriggerStore._load_triggers_from_file",
