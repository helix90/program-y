...
class MongoUploaderTests(unittest.TestCase):
    ...
    @patch(
        "programy.storage.stores.nosql.mongo.loader.Uploader.get_args", patch_get_args
    )
    def test_run(self):
        Uploader.run()
    ...
    @patch(
        "programy.storage.stores.nosql.mongo.loader.Uploader.get_args", patch_get_args2
    )
    ...
    def test_upload_postquestionprocessors(self):
        count, success = Uploader.upload(
            storetype="postquestionprocessors",
            url="mongodb://localhost:27017/",
            database="programy",
            filename=self.basepath
            + "lookups"
