...
class FileSpellingStoreTests(unittest.TestCase):
    ...
    def test_load_spelling(self):
        config = FileStorageConfiguration()
        config._spelling_storage = FileStoreConfiguration(
    ...
    @patch(
        "programy.storage.stores.file.store.spelling.FileSpellingStore._load_corpus_from_file",
        patch_load_corpus_from_file,
    )
