...
from programy.storage.stores.sql.dao.usergroup import (
    AuthoriseGroup,
    AuthoriseUser,
    GroupGroup,
    GroupRole,
...
class UserRoleTests(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertIsNotNone(authuser1)
        self.assertEqual(
            "<UserRole(id='n/a', user='user1', role='admin')>", str(authuser1)
        )

        authuser2 = UserRole(id=1, user="user1", role="admin")
        self.assertIsNotNone(authuser2)
        self.assertEqual(
            "<UserRole(id='1', user='user1', role='admin')>", str(authuser2)
        )
...
class UserGroupTests(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertIsNotNone(authuser1)
        self.assertEqual(
            "<UserGroup(id='n/a', user='user1', group='admin')>", str(authuser1)
        )

        authuser2 = UserGroup(id=1, user="user1", group="admin")
        self.assertIsNotNone(authuser2)
        self.assertEqual(
            "<UserGroup(id='1', user='user1', group='admin')>", str(authuser2)
        )
...
class AuthoriseGroupTests(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertIsNotNone(authuser1)
        self.assertEqual(
            "<AuthoriseGroup(id='n/a', name='group1', parent='group0')>", str(authuser1)
        )

        authuser2 = AuthoriseGroup(id=1, name="group1", parent="group0")
        self.assertIsNotNone(authuser2)
        self.assertEqual(
            "<AuthoriseGroup(id='1', name='group1', parent='group0')>", str(authuser2)
        )
...
class GroupGroupTests(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertIsNotNone(authuser1)
        self.assertEqual(
            "<GroupGroup(id='n/a', group='group1', subgroup='group2')>", str(authuser1)
        )

        authuser2 = GroupGroup(id=1, group="group1", subgroup="group2")
        self.assertIsNotNone(authuser2)
        self.assertEqual(
            "<GroupGroup(id='1', group='group1', subgroup='group2')>", str(authuser2)
        )
...
class GroupRoleTests(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertIsNotNone(authuser1)
        self.assertEqual(
            "<GroupRole(id='n/a', group='group1', role='admin')>", str(authuser1)
        )

        authuser2 = GroupRole(id=1, group="group1", role="admin")
        self.assertIsNotNone(authuser2)
        self.assertEqual(
            "<GroupRole(id='1', group='group1', role='admin')>", str(authuser2)
        )
...
class GroupUserTests(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertIsNotNone(authuser1)
        self.assertEqual(
            "<GroupUser(id='n/a', group='group1', user='user1')>", str(authuser1)
        )

        authuser2 = GroupUser(id=1, group="group1", user="user1")
        self.assertIsNotNone(authuser2)
        self.assertEqual(
            "<GroupUser(id='1', group='group1', user='user1')>", str(authuser2)
        )
