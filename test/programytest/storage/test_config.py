...
class StorageConfigurationTests(unittest.TestCase):
    ...
    def assert_entity_store(self, storage_config, file=True, sqllite=False):
        ...
        if sqllite is True:
            self.assertEqual(
                storage_config.entity_store[StorageFactory.USERS], "sqlite"
            )
            self.assertEqual(
                storage_config.entity_store[StorageFactory.LINKED_ACCOUNTS], "sqlite"
            )
            self.assertEqual(
                storage_config.entity_store[StorageFactory.LINKS], "sqlite"
            )

        if file is True:
            self.assertEqual(
                storage_config.entity_store[StorageFactory.CATEGORIES], "file"
            )
            self.assertEqual(storage_config.entity_store[StorageFactory.ERRORS], "file")
            self.assertEqual(
                storage_config.entity_store[StorageFactory.DUPLICATES], "file"
            )
            self.assertEqual(storage_config.entity_store[StorageFactory.LEARNF], "file")

            self.assertEqual(
                storage_config.entity_store[StorageFactory.CONVERSATIONS], "file"
            )
            ...
            self.assertEqual(
                storage_config.entity_store[StorageFactory.DENORMAL], "file"
            )
            ...
            self.assertEqual(
                storage_config.entity_store[StorageFactory.PERSON2], "file"
            )
            self.assertEqual(
                storage_config.entity_store[StorageFactory.REGEX_TEMPLATES], "file"
            )

            self.assertEqual(
                storage_config.entity_store[StorageFactory.PROPERTIES], "file"
            )
            self.assertEqual(
                storage_config.entity_store[StorageFactory.DEFAULTS], "file"
            )

            self.assertEqual(
                storage_config.entity_store[StorageFactory.TWITTER], "file"
            )

            self.assertEqual(
                storage_config.entity_store[StorageFactory.SPELLING_CORPUS], "file"
            )

            self.assertEqual(
                storage_config.entity_store[StorageFactory.LICENSE_KEYS], "file"
            )

            self.assertEqual(
                storage_config.entity_store[StorageFactory.TEMPLATE_NODES], "file"
            )
            self.assertEqual(
                storage_config.entity_store[StorageFactory.PATTERN_NODES], "file"
            )

            self.assertEqual(
                storage_config.entity_store[StorageFactory.BINARIES], "file"
            )
            self.assertEqual(
                storage_config.entity_store[StorageFactory.BRAINTREE], "file"
            )

            self.assertEqual(
                storage_config.entity_store[StorageFactory.PREPROCESSORS], "file"
            )
            self.assertEqual(
                storage_config.entity_store[StorageFactory.POSTPROCESSORS], "file"
            )

            self.assertEqual(
                storage_config.entity_store[StorageFactory.USERGROUPS], "file"
            )

            self.assertEqual(
                storage_config.entity_store[StorageFactory.TRIGGERS], "file"
            )

            self.assertEqual(storage_config.entity_store[StorageFactory.OOBS], "file")

            self.assertEqual(
                storage_config.entity_store[StorageFactory.SERVICES], "file"
            )

    def assert_storage_configurations(
        self,
        storage_config,
        file=True,
        sqlite=False,
    ...
    def test_create_storage_config_nothing(self):
        ...
        config = storage_config.create_storage_config(
            file=False, sqlite=False, mongo=False, redis=False, logger=False
        )
        self.assertIsNotNone(config)
    ...
    def test_create_storage_config_all(self):
        ...
        config = storage_config.create_storage_config(
            file=True, sqlite=True, mongo=True, redis=True, logger=True
        )
        self.assertIsNotNone(config)
    ...
    def test_add_default_stores_all_on(self):
        store_configs = {}
        StorageConfiguration.add_default_stores_as_yaml(
            store_configs, file=True, sqlite=True, mongo=True, redis=True, logger=True
        )
    ...
    def assertDefaultFileStoreConfig(self, store_configs):
        ...
        self.assertEquals(
            config["learnf_storage"],
            {
                "dirs": ["/tmp/categories/learnf"],
                "extension": "aiml",
                "subdirs": False,
                "format": "xml",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["denormal_storage"],
            {
                "file": "/tmp/lookups/denormal.txt",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["normal_storage"],
            {
                "file": "/tmp/lookups/normal.txt",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["gender_storage"],
            {
                "file": "/tmp/lookups/gender.txt",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["person_storage"],
            {
                "file": "/tmp/lookups/person.txt",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["person2_storage"],
            {
                "file": "/tmp/lookups/person2.txt",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["regex_storage"],
            {
                "file": "/tmp/regex/regex-templates.txt",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["pattern_nodes_storage"],
            {
                "file": "/tmp/nodes/pattern_nodes.conf",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["template_nodes_storage"],
            {
                "file": "/tmp/nodes/template_nodes.conf",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["preprocessors_storage"],
            {
                "file": "/tmp/processing/preprocessors.conf",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["postprocessors_storage"],
            {
                "file": "/tmp/processing/postprocessors.conf",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
        ...
        self.assertEquals(
            config["postquestionprocessors_storage"],
            {
                "file": "/tmp/processing/postquestionprocessors.conf",
                "extension": None,
                "subdirs": False,
                "format": "text",
                "encoding": "utf-8",
    ...
    def assert_defaults(test, data):
        ...
        test.assertEqual(
            data["entities"][StorageFactory.POSTQUESTIONPROCESSORS], "file"
        )
        ...
        FileStorageConfigurationTests.assert_defaults(
            test, data["stores"]["file"]["config"]
        )
