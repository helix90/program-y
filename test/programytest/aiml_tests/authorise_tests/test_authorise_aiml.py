...
class AuthoriseTestClient(TestClient):
    ...
    def load_configuration(self, arguments):
        ...
        self.configuration.client_configuration.configurations[0].configurations[
            0
        ].security._authorisation = BrainSecurityAuthorisationConfiguration()
        self.configuration.client_configuration.configurations[0].configurations[
            0
        ].security.authorisation._classname = "programy.security.authorise.usergroupsauthorisor.BasicUserGroupAuthorisationService"
        self.configuration.client_configuration.configurations[0].configurations[
            0
        ].security.authorisation._denied_srai = "ACCESS_DENIED"
        self.configuration.client_configuration.configurations[0].configurations[
            0
        ].security.authorisation._usergroups = (
...
class AuthoriseAIMLTests(unittest.TestCase):
    ...
    def test_authorise_allowed(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "ALLOW ACCESS"
        )
    ...
    def test_authorise_denied(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "DENY ACCESS"
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            response, "Sorry, but you are not authorised to access this content!"
        )

    def test_authorise_denied_custom_response(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "CUSTOM DENY ACCESS"
        )
        self.assertIsNotNone(response)
