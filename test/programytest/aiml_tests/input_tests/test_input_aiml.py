...
class InputAIMLTests(unittest.TestCase):
    ...
    def test_basic_input(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "TEST INPUT"
        )
    ...
    def test_basic_series(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "QUESTION 1"
        )
        self.assertIsNotNone(response)
        self.assertEqual(response, "Hello 1.")

        response = self._client_context.bot.ask_question(
            self._client_context, "QUESTION 2"
        )
        self.assertIsNotNone(response)
        self.assertEqual(response, "Hello 2.")

        response = self._client_context.bot.ask_question(
            self._client_context, "PREVIOUS QUESTION"
        )
        self.assertIsNotNone(response)
        self.assertEqual(response, "QUESTION 1.")

        response = self._client_context.bot.ask_question(
            self._client_context, "2ND PREVIOUS QUESTION"
        )
        self.assertIsNotNone(response)
