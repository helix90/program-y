...
class LearnfTestClient(TestClient):
    ...
    def load_configuration(self, arguments):
        ...
        self.configuration.client_configuration.configurations[0].configurations[
            0
        ].overrides._allow_learn_aiml = True
        self.configuration.client_configuration.configurations[0].configurations[
            0
        ].overrides._allow_learnf_aiml = True
...
class LearnfAIMLTests(unittest.TestCase):
    ...
    def test_john_played_cricket(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "JOHN PLAYED CRICKET"
        )
        self.assertIsNotNone(response)
        ...
        response = self._client_context.bot.ask_question(
            self._client_context, "WHAT DID JOHN PLAY"
        )
        self.assertIsNotNone(response)
    ...
    def test_lazyracoon(self):
        ...
        response = self._client_context.bot.ask_question(
            self._client_context, "hello IS SYNONYM OF hi"
        )
        self.assertIsNotNone(response)
