...
class PatternSetAIMLTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._client_context.brain.dynamics.add_dynamic_set(
            "roman", "programy.dynamic.sets.roman.IsRomanNumeral", None
        )
        self._client_context.brain.dynamics.add_dynamic_set(
            "stopword", "programy.dynamic.sets.stopword.IsStopWord", None
        )

    def test_patten_set_match(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "MY FAVORITE COLOR IS AMBER"
        )
        self.assertEqual(response, "Amber IS A NICE COLOR.")

    def test_patten_match_multi_word_set(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "MY FAVORITE COLOR IS AIR FORCE BLUE"
        )
    ...
    def test_patten_match_mixed_word_set(self):
        ...
        response = self._client_context.bot.ask_question(
            self._client_context, "MY FAVORITE COLOR IS RED ORANGE"
        )
        self.assertEqual(response, "Red Orange IS A NICE COLOR.")

        response = self._client_context.bot.ask_question(
            self._client_context, "MY FAVORITE COLOR IS SACRAMENTO STATE GREEN"
        )
