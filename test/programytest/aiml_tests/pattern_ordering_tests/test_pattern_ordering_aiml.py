...
class PatternOrderingAIMLTests(unittest.TestCase):
    ...
    def test_basic_no_match(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "MY FAVORITE COLOR IS BLUE"
        )
    ...
    def test_hash_v_star(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "MY FAVORITE ANIMAL IS A DOLPHIN"
        )
        self.assertEqual(response, "HASH SELECTED.")

        response = self._client_context.bot.ask_question(
            self._client_context, "MY FAVORITE ANIMAL IS AN AARDVARK"
        )
