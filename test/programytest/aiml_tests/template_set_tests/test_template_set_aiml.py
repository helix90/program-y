...
class TemplateSetAIMLTests(unittest.TestCase):
    ...
    def test_name_set_topic(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "NAME SET"
        )
        self.assertIsNotNone(response)
        self.assertEqual(response, "OK test1.")
        self.assertEqual(
            self._client_context.bot.get_conversation(self._client_context).property(
                "var1"
            ),
            "test1",
        )
    ...
    def test_multi_word_name_set_topic(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "MULTI WORD NAME SET"
        )
        self.assertIsNotNone(response)
    ...
    def test_multi_word_var_set_topic(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "MULTI WORD VAR SET"
        )
        self.assertIsNotNone(response)
    ...
    def test_topic_set(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "TOPIC SET"
        )
        self.assertIsNotNone(response)
        ...
        response = self._client_context.bot.ask_question(
            self._client_context, "TOPIC UNSET"
        )
        self.assertIsNotNone(response)
    ...
    def test_multi_word_topic_set(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "SET MULTI WORD TOPIC"
        )
        self.assertIsNotNone(response)
    ...
    def test_var_transience(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "VAR TEST STEP1"
        )
        self.assertIsNotNone(response)
        self.assertEqual(response, "STEP1 Val1.")
        response = self._client_context.bot.ask_question(
            self._client_context, "VAR TEST STEP2"
        )
        self.assertIsNotNone(response)
