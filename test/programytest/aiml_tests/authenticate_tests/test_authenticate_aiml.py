...
class AuthenticateTestClient(TestClient):
    ...
    def load_configuration(self, arguments):
        ...
        self.configuration.client_configuration.configurations[0].configurations[
            0
        ].security._authentication = BrainSecurityAuthenticationConfiguration(
            "authentication"
        )
        self.configuration.client_configuration.configurations[0].configurations[
            0
        ].security.authentication._classname = "programytest.aiml_tests.authenticate_tests.test_authenticate_aiml.MockAuthenticationService"
        self.configuration.client_configuration.configurations[0].configurations[
            0
        ].security.authentication._denied_srai = "AUTHENTICATED_FAILED"
...
class AuthenticateAIMLTests(unittest.TestCase):
    ...
    def test_authentication_passed(self):
        ...
        response = self._client_context.bot.ask_question(
            self._client_context, "AUTHENTICATE PASSED"
        )
    ...
    def test_authentication_failed_with_srai(self):
        ...
        self._client_context.brain.security.authentication.configuration._denied_srai = (
            "AUTHENTICATED_FAILED"
        )
        self._client_context.brain.security.authentication.configuration._denied_text = (
            "AUTHENTICATED FAILED TEXT"
        )

        response = self._client_context.bot.ask_question(
            self._client_context, "AUTHENTICATE FAIL SRAI"
        )
        self.assertIsNotNone(response)
    ...
    def test_authentication_failed_with_text(self):
        ...
        response = self._client_context.bot.ask_question(
            self._client_context, "AUTHENTICATE FAIL TEXT"
        )
        self.assertIsNotNone(response)
        self.assertEqual(response, "AUTHENTICATED FAILED TEXT.")
