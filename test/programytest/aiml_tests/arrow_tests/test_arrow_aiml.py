...
class ArrowTestClient(TestClient):
    ...
    def load_storage(self):
        ...
        self.add_single_categories_store(
            os.path.dirname(__file__) + os.sep + "arrow.aiml"
        )
...
class ArrowAIMLTests(unittest.TestCase):
    ...
    def test_arrow_first_multi_word(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "WE SAY HEY"
        )
        self.assertIsNotNone(response)
    ...
    def test_arrow_first_multi_match(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "WE CHECKING MULTI WORD MATCH"
        )
        self.assertIsNotNone(response)
    ...
    def test_arrow_no_multi_word(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "HELLO YOU THERE"
        )
        self.assertIsNotNone(response)
    ...
    def test_arrow_middle_no_word(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "WELL THERE"
        )
        self.assertIsNotNone(response)
        self.assertEqual(response, "ARROW IS.")
    ...
    def test_arrow_middle_multi_word(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "WELL I WAS THERE"
        )
        self.assertIsNotNone(response)
    ...
    def test_test1_testx_issue(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "TEST1 TESTX"
        )
        self.assertIsNotNone(response)
    ...
    def test_test1_test2_issue(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "TEST1 TEST2"
        )
        self.assertIsNotNone(response)
