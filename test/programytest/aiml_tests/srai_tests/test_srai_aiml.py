...
class SraiAIMLTests(unittest.TestCase):
    ...
    def test_predicate_set_srai(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "TEST PREDICATE SET"
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            "Global1 set to Value1 and Local1 set to unknown and Local2 set to Value3.",
            response,
        )

    def test_srai_with_star(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "TEST MULTI SRAI KEITH STERLING"
        )
        self.assertIsNotNone(response)
    ...
    def test_srai_with_included_star(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "TEST INCLUDED SRAI KEITH STERLING"
        )
        self.assertIsNotNone(response)
