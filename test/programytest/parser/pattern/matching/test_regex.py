...
class PatternMatcherRegexTests(PatternMatcherBaseClass):
    ...
    def test_basic_regex_match_as_text(self):
        ...
        self.add_pattern_to_graph(
            pattern="I AM <regex>^LEGION$</regex>", topic="*", that="*", template="1"
        )
    ...
    def test_basic_regex_match_as_pattern(self):
        ...
        self.add_pattern_to_graph(
            pattern='I AM <regex pattern="^LEGION$" />',
            topic="*",
            that="*",
            template="1",
        )
    ...
    def test_basic_regex_match_as_template(self):
        ...
        self.add_pattern_to_graph(
            pattern='I AM <regex template="LEGION" />',
            topic="*",
            that="*",
            template="1",
        )
    ...
    def test_regex_loaded_from_file(self):
        ...
        self._client_context.brain.regex_templates.add_regex(
            "ANYINTEGER", re.compile("^\\d+$", re.IGNORECASE)
        )

        self.add_pattern_to_graph(
            pattern='I AM <regex template="ANYINTEGER" /> YEARS OLD',
            topic="*",
            that="*",
            template="CORRECT",
        )
