...
class PatternMatcherSetTests(PatternMatcherBaseClass):
    ...
    def test_basic_set_match_as_text(self):
        ...
        if self._client_context.brain.sets.contains("SEX") is False:
            self._client_context.brain._sets_collection.add_set(
                "SEX", {"MAN": [["MAN"]], "WOMAN": [["WOMAN"]]}, "teststore"
            )

        self.add_pattern_to_graph(
            pattern="I AM A <set>sex</set>", topic="X", that="Y", template="1"
        )
    ...
    def test_basic_set_match_as_name(self):
        ...
        self.add_pattern_to_graph(
            pattern='I AM A <set name="sex" />', topic="X", that="Y", template="1"
        )
    ...
    def test_multi_word_set_match(self):
        ...
        self.add_pattern_to_graph(
            pattern="I LIKE <set>color</set> *", topic="*", that="*", template="1"
        )
        ...
        context = self.match_sentence(
            "I LIKE RED BURNT OAK MOTOR BIKES", topic="*", that="*"
        )
        self.assertIsNotNone(context)
        self.assertIsNotNone(context.template_node)
        self.assertEqual(
    ...
    def test_basic_set_number_match(self):
        ...
        self.add_pattern_to_graph(
            pattern="I AM <set>number</set> YEARS OLD",
            topic="X",
            that="Y",
            template="1",
        )
    ...
    def test_basic_set_synset_match(self):
        ...
        self.add_pattern_to_graph(
            pattern='I AM A <set name="synset" similar="hack" />',
            topic="*",
            that="*",
            template="OK",
        )
