...
class PatternMatcherBaseClass(ParserTestsBaseClass):
    ...
    def add_pattern_to_graph(self, pattern, topic="*", that="*", template="test"):
        ...
        self._client_context.brain.aiml_parser.pattern_parser.add_pattern_to_graph(
            pattern_element, topic_element, that_element, template_node
        )

    def match_sentence(self, sentence, topic="*", that="*"):

        return self._client_context.brain.aiml_parser.match_sentence(
            self._client_context, Sentence(self._client_context, sentence), topic, that
        )
