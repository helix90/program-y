...
class MyLoopTemplateConditionNode(TemplateConditionNode):
    ...
    def __init__(
        ...
        loop=False,
        condition_type=TemplateConditionNode.BLOCK,
        next_value=None,
        next_value_step=0,
    ):
        TemplateConditionNode.__init__(
            self, name, value, var_type, loop, condition_type
        )
    ...
    def _pre_default_loop_check(self, client_context):
        ...
        if self._next_step_count == self._next_value_step:
            client_context.bot.get_conversation(client_context).set_property(
                self._next_value[0], self._next_value[1]
            )
...
class TemplateConditionType3NodeTests(ParserTestsBaseClass):
    ...
    def test_type3_node_with_loop(self):
        ...
        node = MyLoopTemplateConditionNode(
            condition_type=3, next_value=("name1", "value1"), next_value_step=1
        )
        self.assertIsNotNone(node)
    ...
    @patch(
        "programy.parser.template.nodes.condition.TemplateConditionNode._resolve_type3_to_string",
        patch_resolve_type3_to_string,
    )
