...
from programy.mappings.base import (
    DoubleStringCharSplitCollection,
    DoubleStringPatternSplitCollection,
    SingleStringCollection,
)
...
class MockDoubleStringCharSplitCollection(DoubleStringCharSplitCollection):
    ...
    def process_splits(self, splits, id=None):
        if self._process_splits_success is True:
            return super(MockDoubleStringCharSplitCollection, self).process_splits(
                splits, id
            )
        else:
            return False
...
class MockDoubleStringPatternSplitCollection(DoubleStringPatternSplitCollection):
    ...
    def process_splits(self, splits, id=None):
        if self._process_splits_success is True:
            return super(MockDoubleStringPatternSplitCollection, self).process_splits(
                splits, id
            )
        else:
            return False
...
class TestDoubleStringPatternSplitCollection(unittest.TestCase):
    ...
    def test_double_strings_from_text(self):
        ...
        self.assertTrue(doubles.has_key("key1"))
        self.assertEqual(
            [re.compile("(^key1|key1|key1$)", re.IGNORECASE), "VAL1"],
            doubles.value("key1"),
        )

        self.assertTrue(doubles.has_key("key2"))
        self.assertEqual(
            [re.compile("(^key2|key2|key2$)", re.IGNORECASE), "VAL2"],
            doubles.value("key2"),
        )

        self.assertTrue(doubles.has_key("key3"))
        self.assertEqual(
            [re.compile("(^key3|key3|key3$)", re.IGNORECASE), "VAL3,VAL4"],
            doubles.value("key3"),
        )
    ...
    def test_replace_by_pattern(self):
        ...
        self.assertEquals(
            "VAL2 HELLO VAL2", doubles.replace_by_pattern("KEY2 HELLO KEY2")
        )
    ...
    def test_split_line_by_pattern(self):
        ...
        self.assertEquals(
            ["KEY1", "VAL1"],
            doubles.split_line_by_pattern(
                '"KEY1","VAL1"', DoubleStringPatternSplitCollection.RE_OF_SPLIT_PATTERN
            ),
        )
