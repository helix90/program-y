...
class FileFinderTests(unittest.TestCase):
    ...
    def test_find_files_no_subdir(self):
        ...
        files = file_finder.find_files(
            os.path.dirname(__file__) + os.sep + "test_files",
            subdir=False,
            extension=".txt",
        )
    ...
    def test_find_files_subdir_bad_dir(self):
        file_finder = MockFileFinder()
        files = file_finder.find_files(
            os.path.dirname(__file__) + os.sep + "test_filesXXX",
            subdir=True,
            extension=".txt",
        )
    ...
    def test_load_dir_contents_no_subdir_filename_as_userid_false(self):
        file_finder = MockFileFinder()
        collection, file_maps = file_finder.load_dir_contents(
            os.path.dirname(__file__) + os.sep + "test_files",
            subdir=False,
            extension=".txt",
    ...
    def test_load_single_file_contents(self):
        file_finder = MockFileFinder()
        collection = file_finder.load_single_file_contents(
            os.path.dirname(__file__) + os.sep + "test_files" + os.sep + "file1.txt"
        )
    ...
    def test_get_just_filename_from_filepath(self):
        ...
        self.assertEquals(
            "filename", FileFinder.get_just_filename_from_filepath("filename")
        )
        self.assertEquals(
            "filename", FileFinder.get_just_filename_from_filepath("filename.txt")
        )
        self.assertEquals(
            "filename",
            FileFinder.get_just_filename_from_filepath("somepath/otherpath/filename"),
        )
        self.assertEquals(
            "filename",
            FileFinder.get_just_filename_from_filepath(
                "somepath/otherpath/filename.txt"
            ),
        )
        self.assertEquals(
            "filename",
            FileFinder.get_just_filename_from_filepath(
                "somepath/otherpath/filename.txt.pdf"
            ),
        )
