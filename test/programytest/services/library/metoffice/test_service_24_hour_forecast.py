...
class MetOffice24HourForecastServiceTests(ServiceTestCase):
    ...
    def _do_24_hour_forecast(self):
        service = MetOfficeService(
            ServiceConfiguration.from_data("rest", "metoffice", "weather")
        )
    ...
    @patch(
        "programy.services.library.metoffice.metoffice.MetOffice.twentyfour_hour_forecast",
        patch_metoffer_24_hour_forecast_success,
    )
    ...
    @patch(
        "programy.services.library.metoffice.service.MetOfficeHoursForecastQuery._get_current_date",
        patch_get_current_date,
    )
    @patch(
        "programy.services.library.metoffice.metoffice.MetOffice.twentyfour_hour_forecast",
        patch_metoffer_24_hour_forecast_success,
    ...
    def test_handler_load_24_hour_forecast(self):
        ...
        response = self._do_handler_load(
            ...
            conf_file,
            "metoffice",
            "METOFFICE FORECAST LAT SIGN POS DEC 56 FRAC 0719912 LNG SIGN NEG DEC 3 FRAC 1750909 HOURS 3",
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            "It will be a Sunny day , with a temperature of 4 'C.", response
        )
