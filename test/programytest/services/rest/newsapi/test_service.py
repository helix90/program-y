...
from programytest.services.rest.newsapi.responses import (
    everything_success_response,
    headlines_success_response,
    sources_success_response,
)
...
class NewsAPIServiceTests(ServiceTestCase):
    ...
    def test_init(self):
        service = NewsAPIService(
            ServiceConfiguration.from_data("rest", "newsapi", "news")
        )
    ...
    @patch(
        ...
        patch_requests_everything_success_response,
    )
    ...
    @patch(
        "programy.services.rest.base.RESTService._requests_get",
        patch_requests_headlines_success_response,
    )
    ...
    @patch(
        "programy.services.rest.base.RESTService._requests_get",
        patch_requests_sources_success_response,
    )
    ...
    def test_handler_load_everything(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "newsapi", "NEWSAPI EVERYTHING CHATBOTS"
        )
        self.assertIsNotNone(response)
        self.assertTrue(
            response.startswith(
                "<ul><li>How artificial intelligence and machine learning produced robots we can talk to</li>"
            )
        )
