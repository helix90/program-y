...
from programytest.services.rest.getguidelines.responses import (
...
class GetGuidelinesServiceTests(ServiceTestCase):
    ...
    @patch(
        ...
        patch_requests_all_success_response,
    )
    ...
    def test_all_aiml(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "getguidelines", "GETGUIDELINES ALL DM CHF"
        )
        self.assertIsNotNone(response)
        self.assertTrue(
            response.startswith(
                "<ol><li>#1: Monitor glucose, HbA1c, blood pressure, body weight,"
            )
        )

    @patch(
        "programy.services.rest.base.RESTService._requests_get",
        patch_requests_vac_success_response,
    )
    def test_vac_aiml(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "getguidelines", "GETGUIDELINES VACINATION DM CHF"
        )
        self.assertIsNotNone(response)
    ...
    def test_conditions_aiml(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "getguidelines", "GETGUIDELINES CONDITIONS"
        )
        self.assertIsNotNone(response)
