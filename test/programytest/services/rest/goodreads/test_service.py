...
from programytest.services.rest.goodreads.responses import (
    book_list_success_response,
    search_author_success_response,
)
...
class GoodReadsServiceTests(ServiceTestCase):
    ...
    @unittest.skip(
        "Goodreads currently disabled at source - https://help.goodreads.com/s/article/Why-did-my-API-key-stop-working"
    )
    def test_init(self):
        service = GoodReadsService(
            ServiceConfiguration.from_data("rest", "goodreads", "books")
        )
    ...
    @patch(
        ...
        patch_requests_search_author_success_response,
    )
    ...
    @patch(
        "programy.services.rest.base.RESTService._requests_get",
        patch_requests_book_list_success_response,
    )
    ...
    def test_handler_load_author_search(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "goodreads", "GOODREADS SEARCH AUTHOR IAIN BANKS"
        )
    ...
    def test_handler_load_author_search(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "goodreads", "GOODREADS BOOK LIST 9226"
        )
        self.assertIsNotNone(response)
