...
class OMDBServiceTests(ServiceTestCase):
    ...
    @patch(
        ...
        patch_requests_title_search_success,
    )
    ...
    def test_handler_title_search(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "omdb", "OMDB TITLE SEARCH ALIENS"
        )
        self.assertIsNotNone(response)
        self.assertTrue(
            response.startswith(
                "Aliens was released in 18 Jul 1986 and directed by James Cameron and starring Sigourney Weaver"
            )
        )
