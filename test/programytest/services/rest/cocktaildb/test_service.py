...
from programytest.services.rest.cocktaildb.responses import (
    search_by_ingredient_success_response,
    search_by_name_success_response,
)
...
class CocktailDBServiceTests(ServiceTestCase):
    ...
    def test_handler_load_name(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "cocktaildb", "COCKTAILDB SEARCH NAME OLD FASHIONED"
        )
    ...
    def test_handler_load_ingredient(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "cocktaildb", "COCKTAILDB SEARCH INGREDIENT BOURBON"
        )
        self.assertIsNotNone(response)
        self.assertTrue(
            response.startswith(
