...
from programytest.services.rest.duckduckgo.responses import (
    instant_success_response,
...
class DuckDuckGoServiceTests(ServiceTestCase):
    ...
    def test_instant_aiml(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "duckduckgo", "DUCKDUCKGO INSTANT CHATBOTS"
        )
    ...
    def test_scrape_aiml(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "duckduckgo", "DUCKDUCKGO SCRAPE WHAT ARE CHATBOTS"
        )
        self.assertTrue(
            response.startswith(
                "A chatbot is a piece of software that conducts a conversation via auditory or textual methods"
