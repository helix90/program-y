...
class GoogleGeoCodeServiceTests(ServiceTestCase):
    ...
    def test_init(self):
        service = GoogleGeoCodeService(
            ServiceConfiguration.from_data("rest", "google", "geocode")
        )
    ...
    @patch(
        ...
        patch_requests_postcode_valid_response,
    )
    ...
    def test_handler_load(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "google.geocode", "GOOGLE LATLNG POSTCODE KY39UR"
        )
        self.assertIsNotNone(response)
        self.assertEquals(
            "LAT SIGN POS DEC 56 FRAC 0719912 LNG SIGN NEG DEC 3 FRAC 1750909.",
            response,
        )
