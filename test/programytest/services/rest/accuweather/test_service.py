...
from programytest.services.rest.accuweather.responses import (
    conditions_success_response,
    postcodesearch_success_response,
    textsearch_success_response,
)
...
class AccuWeatherServiceTests(ServiceTestCase):
    ...
    def test_init(self):
        service = AccuWeatherService(
            ServiceConfiguration.from_data("rest", "omdb", "film")
        )
    ...
    def _do_postcodesearch(self):
        service = AccuWeatherService(
            ServiceConfiguration.from_data("rest", "accuweather", "weather")
        )
        self.assertIsNotNone(service)
    ...
    @patch(
        ...
        patch_requests_postcodesearch_success,
    )
    ...
    @patch(
        "programy.services.rest.base.RESTService._requests_get",
        patch_requests_textsearch_success,
    )
    ...
    @patch(
        "programy.services.rest.base.RESTService._requests_get",
        patch_requests_conditions_success,
    )
    ...
    def test_handler_load_textsearch(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "accuweather", "ACCUWEATHER TEXTSEARCH LOCATION KINGHORN"
        )
    ...
    def test_conditions_location_aiml(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "accuweather", "ACCUWEATHER CONDITIONS LOCATION KINGHORN"
        )
        self.assertIsNotNone(response)
    ...
    def test_weather_location_aiml(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "accuweather", "ACCUWEATHER WEATHER LOCATION KINGHORN"
        )
        self.assertIsNotNone(response)
