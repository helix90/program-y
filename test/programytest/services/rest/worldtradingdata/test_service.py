...
from programytest.services.rest.worldtradingdata.responses import (
...
from programy.services.rest.worldtradingdata.service import (
    WorldTradingDataStocksService,
)
...
class WorldTradingDataStocksServiceTests(ServiceTestCase):
    ...
    def test_init(self):
        service = WorldTradingDataStocksService(
            ServiceConfiguration.from_data("rest", "worldtradingdata", "stockmarket")
        )
    ...
    def test_handler_load(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "worldtradingdata", "WORLDTRADINGDATA SYMBOLS AMZN"
        )
