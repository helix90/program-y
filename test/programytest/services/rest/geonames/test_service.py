...
from programytest.services.rest.geonames.responses import (
    placename_success_response,
    postcode_success_response,
)
...
class GeoNamesServiceTests(ServiceTestCase):
    ...
    def test_init(self):
        service = GeoNamesService(
            ServiceConfiguration.from_data("rest", "geonames", "geocode")
        )
    ...
    @patch(
        "programy.services.rest.base.RESTService._requests_get",
        patch_requests_placename_success_reponse,
    )
    ...
    def test_postcode_aiml(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "geonames", "GEONAMES LATLNG POSTCODE KY39UR"
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            "LAT SIGN POS DEC 56 FRAC 07206630948395 LNG SIGN NEG DEC 3 FRAC 175233081708717.",
            response,
        )
    ...
    def test_placename_aiml(self):
        ...
        response = self._do_handler_load(
            client, conf_file, "geonames", "GEONAMES LATLNG PLACENAME KINGHORN"
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            "LAT SIGN POS DEC 56 FRAC 07206630948395 LNG SIGN NEG DEC 3 FRAC 175233081708717.",
    ...
    def test_placename_aiml_with_country(self):
        ...
        response = self._do_handler_load(
            ...
            conf_file,
            "geonames",
            "GEONAMES LATLNG PLACENAME KINGHORN COUNTRY UK",
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            "LAT SIGN POS DEC 56 FRAC 07206630948395 LNG SIGN NEG DEC 3 FRAC 175233081708717.",
    ...
    def test_placename_aiml_different_country(self):
        ...
        response = self._do_handler_load(
            ...
            conf_file,
            "geonames",
            "GEONAMES LATLNG PLACENAME WASHINGTON COUNTRY US",
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            "LAT SIGN POS DEC 38 FRAC 545851 LNG SIGN NEG DEC 91 FRAC 019346.", response
        )
        ...
        response = self._do_handler_load(
            ...
            conf_file,
            "geonames",
            "GEONAMES LATLNG PLACENAME WASHINGTON COUNTRY UK",
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            "LAT SIGN POS DEC 54 FRAC 885019095463335 LNG SIGN NEG DEC 1 FRAC 5427184694082379.",
            response,
        )
