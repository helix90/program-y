...
class SentenceTests(unittest.TestCase):
    ...
    def test_split_into_words1(self):
        self.assertEqual(
            [], Sentence._split_into_words(self._client_context.brain.tokenizer, [])
        )

    def test_split_into_words2(self):
        self.assertEqual(
            [], Sentence._split_into_words(self._client_context.brain.tokenizer, None)
        )
    ...
    def test_split_into_words5(self):
        self.assertEqual(
            ["word1"],
            Sentence._split_into_words(self._client_context.brain.tokenizer, "word1"),
        )
    ...
    def test_split_into_words6(self):
        self.assertEqual(
            ...
            Sentence._split_into_words(
                self._client_context.brain.tokenizer, "word1 word2 word3"
            ),
        )
    ...
    def test_words_from_current_pos(self):
        ...
        self.assertIsNotNone(sentence)
        self.assertEqual(
            "One Two Three", sentence.words_from_current_pos(self._client_context, 0)
        )
        self.assertEqual(
            "Two Three", sentence.words_from_current_pos(self._client_context, 1)
        )
        self.assertEqual(
        ...
        with self.assertRaises(Exception):
            self.assertEqual(
                "Three", sentence.words_from_current_pos(self._client_context, 3)
            )
    ...
    def test_to_json(self):
        ...
        sentence = Sentence(
            self._client_context, "One Two Three", matched_context=context
        )
    ...
    def test_from_json(self):
        ...
        json_data = {
            ...
            "matched_context": {
                "max_search_depth": 100,
                "max_search_timeout": 60,
                "total_search_start": datetime.datetime(
                    2019, 8, 30, 7, 36, 54, 928185
