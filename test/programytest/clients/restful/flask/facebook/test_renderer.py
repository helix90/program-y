...
class FacebookRendererTests(unittest.TestCase):
    ...
    def test_handle_url_button(self):
        button = {
            "type": "button",
            "text": "Servusai",
            "url": "https://www.servusai.com",
        }
        self.renderer.handle_url_button(self.client_context, button)
        self.assertIsNotNone(self.renderer._payload)
        self.assertEqual("testid", self.renderer._payload["recipient"]["id"])
        self.assertEqual(
            "template", self.renderer._payload["message"]["attachment"]["type"]
        )
        self.assertEqual(
            "button",
            self.renderer._payload["message"]["attachment"]["payload"]["template_type"],
        )
        self.assertEqual(
            "Servusai",
            self.renderer._payload["message"]["attachment"]["payload"]["text"],
        )
        self.assertEqual(
            1,
            len(self.renderer._payload["message"]["attachment"]["payload"]["buttons"]),
        )
        self.assertEqual(
            "web_url",
            self.renderer._payload["message"]["attachment"]["payload"]["buttons"][0][
                "type"
            ],
        )
        self.assertEqual(
            "https://www.servusai.com",
            self.renderer._payload["message"]["attachment"]["payload"]["buttons"][0][
                "url"
            ],
        )
    ...
    def test_handle_postback_button(self):
        button = {"type": "button", "text": "Servusai", "postback": "Servusai"}
    ...
    def test_handle_link(self):
        link = {"type": "link", "text": "Servusai", "url": "https://www.servusai.com"}
    ...
    def test_handle_image(self):
        image = {"type": "image", "url": "https://www.servusai.com/test.png"}
        ...
        self.assertEqual(
            "image",
            self.renderer._payload["message"]["attachment"]["payload"]["elements"][0][
                "media_type"
            ],
        )
        self.assertEqual(
            "https://www.servusai.com/test.png",
            self.renderer._payload["message"]["attachment"]["payload"]["elements"][0][
                "url"
            ],
        )
    ...
    def test_handle_video(self):
        video = {"type": "image", "url": "https://www.servusai.com/test.mov"}
        ...
        self.assertEqual(
            "https://www.servusai.com/test.mov",
            self.renderer._payload["message"]["attachment"]["payload"]["elements"][0][
                "url"
            ],
        )
            ...
            "buttons": [
                {
                    "type": "button",
                    "text": "Servusai",
                    "url": "http://www.servusai.com",
                },
                {
                    "type": "button",
                    "text": "AIML Foundation",
    ...
    def test_handle_reply(self):
        reply = {"type": "reply", "text": "Servusai", "postback": None}
    ...
    def test_handle_postback_reply(self):
        reply = {"type": "reply", "text": "Servusai", "postback": "SERVUSAI"}
        self.renderer.handle_reply(self.client_context, reply)
        self.assertIsNotNone(self.renderer._payload)
        self.assertEqual("testid", self.renderer._payload["recipient"]["id"])
        self.assertEqual(
    ...
    def test_handle_location(self):
        ...
        self.assertEqual(
            "location",
            self.renderer._payload["message"]["quick_replies"][0]["content_type"],
        )
