...
class SentimentTestClient(TestClient):
    ...
    def load_configuration(self, arguments):
        ...
        self.configuration.client_configuration.configurations[
            0
        ]._sentiment._classname = (
            "programy.nlp.sentiment.textblob_sentiment.TextBlobSentimentAnalyser"
        )
        self.configuration.client_configuration.configurations[0]._sentiment._scores = (
...
class SentimentAIMLTests(unittest.TestCase):
    ...
    def test_conversation_sentiment(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "I LOVE FRED"
        )
        self.assertIsNotNone(response)
        self.assertEqual(response, "Done.")

        response = self._client_context.bot.ask_question(
            self._client_context, "CONVERSATION SENTIMENT"
        )
        self.assertIsNotNone(response)
    ...
    def test_conversation_sentiment(self):
        ...
        response = self._client_context.bot.ask_question(
            self._client_context, "CURRENT SENTIMENT NUMERIC"
        )
        self.assertIsNotNone(response)
        self.assertEqual(response, "SENTIMENT SCORES POSITIVITY 0.5 SUBJECTIVITY 0.6.")

        response = self._client_context.bot.ask_question(
            self._client_context, "CURRENT SENTIMENT TEXT"
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            response,
    ...
    def test_negative_sentiment(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "I REALLY REALLY HATE YOU"
        )
        self.assertIsNotNone(response)
        self.assertEqual(response, "Done.")
    ...
    def test_calc_sentiment_score(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "CALCULATE SENTIMENT SCORE I REALLY REALLY HATE YOU"
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            response,
    ...
    def test_calc_sentiment_feeling_last_n(self):
        ...
        response = self._client_context.bot.ask_question(
            self._client_context, "CALCULATE SENTIMENT FEELING LAST 1"
        )
        self.assertIsNotNone(response)
    ...
    def test_calc_sentiment_feeling_overall(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "CALCULATE SENTIMENT FEELING OVERALL"
        )
        self.assertIsNotNone(response)
        self.assertEqual(
            response, "SENTIMENT FEELING NEUTRAL AND COMPLETELY OBJECTIVE."
        )

    def test_get_positivity_score(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "GET SENTIMENT POSITIVITY 0 dot 0"
        )
        self.assertIsNotNone(response)
    ...
    def test_sentiment_enabled(self):
        response = self._client_context.bot.ask_question(
            self._client_context, "IS SENTIMENT ENABLED"
        )
        self.assertIsNotNone(response)
